...
async def auto_suspend_job():
    ...
    async with AsyncSessionLocal() as db:
        ...
        stmt_expired = select(Customer).where(
            Customer.status == CustomerStatus.ACTIVE,
            Customer.expires_at < now_utc
        )
        ...
        stmt_unpaid = select(Invoice).where(
        ...
        for inv in unpaid_invoices:
            ...
            if cust and cust.status == CustomerStatus.ACTIVE:
                cust.status = CustomerStatus.SUSPENDED
                logger.info(f"Auto-suspend pelanggan {cust.radius_username} karena menunggak tagihan {inv.invoice_number}.")
                await kick_active_session(db, cust.radius_username)
        
        await db.commit()
    logger.info("Auto-Suspend Job selesai.")

async def kick_active_session(db: AsyncSession, username: str):
