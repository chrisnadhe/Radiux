...
async def generate_monthly_invoices(db: AsyncSession) -> int:
    ...
    for cust in customers:
        ...
        exist_stmt = select(Invoice).where(Invoice.customer_id == cust.id, Invoice.billing_period == billing_period)
        ...
        try:
            ...
            logger.warning(
                f"Saldo tenant {cust.tenant_id} tidak cukup untuk invoice customer {cust.id}. Suspending customer."
            )
...
async def create_manual_invoice(
    db: AsyncSession, customer_id: int, amount: float, billing_period: str, due_date: date, tenant_id: int
) -> Invoice:
    ...
    if tenant_id != 1:
        ...
        await wallet_service.deduct_balance(
            db=db, tenant_id=tenant_id, amount=amount, notes=f"Manual invoice customer {customer_id}"
        )
