...
from app.models.wallet_transactions import WalletTransaction, TransactionType
...
async def add_balance(db: AsyncSession, tenant_id: int, amount: float | Decimal, reference: str | None = None, notes: str | None = None) -> WalletTransaction:
...
async def deduct_balance(db: AsyncSession, tenant_id: int, amount: float | Decimal, reference: str | None = None, notes: str | None = None) -> WalletTransaction:
    """Memotong saldo reseller (Deduction). Akan melempar InsufficientBalanceError jika saldo tidak cukup."""
...
async def _process_transaction(
    ...
    if transaction_type == TransactionType.TOPUP:
        ...
        if balance_before < amount_decimal:
            raise InsufficientBalanceError(f"Saldo tidak cukup. Saldo saat ini: {balance_before}, dibutuhkan: {amount_decimal}")
        balance_after = balance_before - amount_decimal
