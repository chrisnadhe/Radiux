...
class AdminUserCreate(BaseModel):
    ...
    @classmethod
    def empty_str_to_none(cls, v: Any) -> Any:  # noqa: ANN401
        if v == "":
            return None
        return v
...
async def list_admin_users(user: CurrentUser, db: AsyncSession = Depends(get_db)) -> Sequence[AdminUser]:
...
async def create_admin_user(req: AdminUserCreate, user: CurrentUser, db: AsyncSession = Depends(get_db)) -> AdminUser:
    """Buat admin user baru."""
    # Hanya superadmin yang bisa membuat user lintas tenant.
    # Reseller hanya bisa membuat operator/viewer untuk tenantnya.
    if not user.is_superadmin:
        if req.tenant_id != user.tenant_id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Tidak bisa membuat user untuk tenant lain"
            )
        if req.role in [AdminRole.SUPERADMIN, AdminRole.RESELLER]:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Hanya Superadmin yang bisa membuat role ini"
            )
...
async def update_admin_user(
    user_id: int, req: AdminUserUpdate, user: CurrentUser, db: AsyncSession = Depends(get_db)
) -> AdminUser:
    """Update admin user."""
    target_user = await db.scalar(
        select(AdminUser).options(joinedload(AdminUser.tenant)).where(AdminUser.id == user_id)
    )
    if not target_user:
